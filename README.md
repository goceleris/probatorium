...
Registered predicates judged only where their data source exists (listed under `properties_not_instrumented` in every other cell, never counted as passed): `I-MW-*` (only the refapps that install the middleware), `I-RFC-1`/`I-RFC-2` (the wire-reading conformance walker, at concurrency ≥ 4), `I-CONN-1` (the refapps' per-connection last-byte table), `I-CHECKPTR` (only a `-tags=checkptr` refapp, which the checkptr tier builds), `I-MEM-2` (only cells of 20 min or more, which idle the refapp twice: after a 60 s burst and again at the end; the 1 h soak cells do, the 150 s nightly cells do not). Still **not instrumented** anywhere: `I-RACE` (`-race` needs cgo, which the `CGO_ENABLED=0` cross-compile forbids), `I-ENG-IOURING` (its counter exists only in a `-tags=validation` build of celeris), `I-DRV` (needs the validator's driver shadow map).
