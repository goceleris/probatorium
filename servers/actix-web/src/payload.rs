...
//
// Verbatim duplicate of servers/axum/src/payload.rs — kept inline (~80
// LOC) rather than extracted to a path-dep crate so each adapter stays
// self-contained and tarball-able for ansible/tasks/build_native_
// competitor.yml. The duplication is bounded and the sizes asserted by
// unit tests, so drift is caught at `cargo test`-time before a cluster
// run reaches the conformance harness.
//
