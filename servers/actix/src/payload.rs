// Deterministic 1 KiB / 8 KiB / 16 KiB / 64 KiB JSON payload generator.
//
...
//
// We emit the bytes by hand rather than via serde_json + a derived
...
//   2. The payload corpus is tiny and entirely ASCII (no escapes needed
//      beyond what manual formatting handles), so the manual path is both
...
fn generate(target_size: usize) -> Vec<u8> {
    ...
    loop {
        ...
        // Tentative size = current buf + footer length. The Go code marshals
        // the whole thing on every iteration, but we can avoid the recompute
        // since the footer is fixed-length.
...
// push_u64 appends the decimal representation of n. Allocation-free vs
// format!("{}", n) — the only "hot" path here on startup, so worth being
// tidy about.
