...
// (the `tls:` option). There is no cleartext-h2 switch on Bun.serve as of
// Bun 1.3.14 — verified via `bun --help` (the only http2 flag is
// `--experimental-http2-fetch`, which is the *client* h2-over-TLS-ALPN
// path, not a server option). So to serve h2c prior-knowledge we drop to
// Bun's node:http2 compatibility layer, whose `http2.createServer()`
// stands up a cleartext h2 listener. We then bridge each h2 stream to the
// framework's WHATWG `fetch` handler so the route table and payloads stay
// byte-identical to the Bun.serve (h1) fast path.
//
// Bun-specific gotcha
// -------------------
...
// FetchHandler is the WHATWG handler shape both Hono (app.fetch) and the
// other Bun adapters expose: a Request in, a Response (or Promise) out.
...
async function dispatch(
  ...
  try {
    ...
    // Copy non-pseudo headers across so the framework sees the real
