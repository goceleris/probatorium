...
func TestTier1Walker_PredicatesRegistered(t *testing.T) {
	for _, want := range []string{"I-ADV-ACCEPTED", "I-H2C-CRASHED", "I-WS-ACCEPTED", "I-WS-HANG", "I-H2C-HANG", "I-WS-HANDSHAKE", "I-LIVENESS", "I-HANG"} {
		spec, ok := ByID(want)
		if !ok {
...
func TestTier1Walker_ByTierFiltersCorrectly(t *testing.T) {
	...
	if len(got) != 8 {
		t.Errorf("ByTier(tier-1-walker): got %d specs, want 8", len(got))
	}
	for _, s := range got {
